...
int main(void)
{
	...
	GLuint programID = LoadShaders("shaders/simple_vs.shader", "shaders/simple_fs.shader");
	...
	static GLuint buffer_size;

	cube.get_buffer(g_vertex_buffer_data, &buffer_size);
	...
	do {
		...
		// Draw the triangle !
		glDrawArrays(GL_TRIANGLES, 0, cube.get_buff_size()/3); // 3 indices starting at 0 -> 1 triangle
